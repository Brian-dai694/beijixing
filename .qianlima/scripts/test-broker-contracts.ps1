...
$powerShellHost = Join-Path $PSHOME $(if ($env:OS -eq 'Windows_NT') { 'powershell.exe' } else { 'pwsh' })
if (-not (Test-Path -LiteralPath $powerShellHost -PathType Leaf)) { throw "Current PowerShell host was not found: $powerShellHost" }
$grant = & $powerShellHost -NoProfile -ExecutionPolicy Bypass -File $grantScript -GrantId $grantId -AgentId 'evidence_checker' -TaskId $taskId -WorkOrderId $orderId -DataRef 'artifact-sanitized' -AllowedTool 'read_selected_sources' -RiskCeiling L2 -VerifierAgentId 'evidence_checker' -PassThru | ConvertFrom-Json
...
$badL4 = & $powerShellHost -NoProfile -ExecutionPolicy Bypass -File $grantScript -GrantId "broker-smoke-l4-$stamp" -AgentId 'executor' -TaskId $taskId -WorkOrderId $orderId -DataRef 'artifact-sanitized' -AllowedTool 'approved_tool_only' -RiskCeiling L4 -VerifierAgentId 'evidence_checker' 2>&1
$badL4Exit = $LASTEXITCODE
$duplicate = & $powerShellHost -NoProfile -ExecutionPolicy Bypass -File $grantScript -GrantId $grantId -AgentId 'evidence_checker' -TaskId $taskId -WorkOrderId $orderId -DataRef 'artifact-sanitized' -AllowedTool 'read_selected_sources' -RiskCeiling L2 -VerifierAgentId 'evidence_checker' 2>&1
