...
foreach($field in @('claim_pack_id','fusion_id','task_id','producer_model_id','producer_model_version','source_refs','data_time_range','assumptions','uncertainties','falsifiers','method_ref','status')){if($null -eq $pack.$field -or [string]::IsNullOrWhiteSpace([string]$pack.$field)){[void]$issues.Add($field+'_required')}}
