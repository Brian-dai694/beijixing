...
$owner = @($roles.governance_roles | Where-Object { $_.id -eq 'business_owner' }) | Select-Object -First 1
$security = @($roles.governance_roles | Where-Object { $_.id -eq 'security_admin' }) | Select-Object -First 1
Add-Case 'owner_not_platform_admin' ($owner.platform_admin -eq $false -and $owner.may_approve_own_request -eq $false)
