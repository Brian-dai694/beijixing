...
这一部分采用工作场景治理思路。北极星的核心治理对象不是代码仓库，而是企业团队的真实工作场景。
...
企业试点不需要一开始启用全部业务能力。先用少量高价值规则跑顺，再经评测和审批逐步增加。
...
企业试点默认使用 `single` 和 `daily`，等流程稳定后再经审批启用 `monitor`。
...
早期企业试点可先用轻量的 `work-hub.ws` 做跨场景索引，再按规模引入受控事件总线。
