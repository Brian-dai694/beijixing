...
├─ .qianlima/              共享治理核心与合同
├─ editions/
│  ├─ personal/            个人版清单、配置、文档和测试
│  └─ enterprise/          企业版清单、配置、文档和测试
├─ start-qianlima.ps1      Windows 共享启动入口
├─ start-qianlima.sh       macOS/Linux 共享启动入口
└─ repository-layout.json  版本分离合同
```

个人版和企业版不互相继承配置，也不复制主 Harness。两者只读引用同一份共享核心，避免安全规则和运行时逐渐分叉。

## 选择版本

### 个人版

适合个人本地工作区。强调快速对话、本地隐私、可查看和删除的个人偏好、受限 Skill 安装，以及高风险动作确认。

查看 [个人版说明](editions/personal/README.md)。

### 企业版

适合员工、部门和多 Agent 协作。增加组织身份、设备、任务 Grant、企业数据准入、MCP 治理、职责分离、审批路由、审计、灰度和回滚。

查看 [企业版说明](editions/enterprise/README.md) 和 [分层使用说明书](editions/enterprise/企业版分层使用说明书.md)。
...
powershell -NoProfile -ExecutionPolicy Bypass -File ".\editions\personal\test-personal-profile.ps1" -PassThru
