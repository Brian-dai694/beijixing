...
## [v2.14.0] - 2026-07-23
- 增加企业智能能力路径和五组业务度量。
- 新增独立的 `C/R/A/I/O` 成熟度合同，与 `L0-L4` 任务风险等级分离。
- 明确 `I` 必须有实验、证据、反例和修正闭环，`O` 必须有组织级协同与治理证据；技术组件本身不能证明成熟度。

## [v2.13.0] - 2026-07-23
- 将 ETCLOVG 固化为企业生产验收矩阵：Execution、Tooling、Context、Lifecycle、Observability、Verification、Governance 七层缺一即阻断发布。
- 新增 Task Runtime Spec 双层合同：自然语言可读工作流负责解释，机器策略负责强制身份、Grant、沙箱、数据、预算、审批、审计和撤销。
- 新增离线 Task Runtime Spec Gate，规格本身不产生执行权；L4 只进入人工候选，外部写入继续关闭。
- Meta-Harness 仅能生成候选并走静态检查、冻结回放、评分、独立核验、人工审查、灰度和回滚；禁止自动修改治理核心。
- 新增 9 项离线回归，覆盖缺 Grant、自核验、状态机缺失、L4 确认缺失、自动应用和风险规则修改阻断。
- HARNESS-LM 的特定实验指标不作为北极星 SLA，仅采纳“低成本授权召回 + 高质量证据复核”的两段式结构。
